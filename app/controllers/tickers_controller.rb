...
class TickersController < ApplicationController
  ...
  def show
    ...
    url="https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY_ADJUSTED&symbol=#{ @ticker.symbol }&apikey=VJFO0T4UA0YKZUT8"
  ...
  def create_row
    ...
    @ticker.url = "https://finance.yahoo.com/quote/#{ @ticker.symbol }"
    @ticker.save
    
    # Save the ticker to UserTickerlist as well
    @user_tickerlist = UserTickerlist.new

    @user_tickerlist.user_id = params.fetch("user_id")
    @user_tickerlist.ticker_id = @ticker.id

    if @user_tickerlist.valid?
      @user_tickerlist.save

      redirect_to("/tickers/#{ @ticker.id }", :notice => "Ticker added to your database successfully.")
    else
      render("ticker_templates/new_form.html.erb")
    end
  end
  ...
  def show_tickerlist
    render("tickers_templates/stock_list.html.erb")
  end
  ...
  def show_stock_info
      ...
      render("/ticker_templates/stock_info.html.erb")
  end
